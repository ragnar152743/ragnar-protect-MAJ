...
class RagnarProtectApp(tk.Tk):
    ...
    def _apply_branding(self) -> None:
        if LOGO_PNG.exists():
            try:
                self.window_icon_image = tk.PhotoImage(file=str(LOGO_PNG))
                self.iconphoto(True, self.window_icon_image)
                self.logo_image = self.window_icon_image.subsample(6, 6)
            except tk.TclError:
                self.logo_image = None
                self.window_icon_image = None
        if LOGO_ICON.exists():
    ...
    def _build_header(self, frame: ttk.Frame) -> None:
        ...
        ttk.Label(title_block, text="Ragnar Protect", font=("Segoe UI Semibold", 22)).pack(anchor="w")
        ttk.Label(
            title_block,
